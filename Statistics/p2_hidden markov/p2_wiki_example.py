...
def updateEmission(n_states, n_emissions, init, transition, emission):
    state_key = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    emission_key = "0123456789"
    emission_state_dic = {}
    ...
    for k,i in emission_state_dic.items():
        ...
        numerator, denominator = 0.0, 0.0
        for s in tran_states:
            for k2,i2 in emission_state_dic[k].items():
...
def normalize_prob_dic(n_states, d):
    state_key = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    unnormalized = []
    for i in range(n_states):
        unnormalized.append(d[state_key[i]])
    normalized = sklearn.preprocessing.normalize(np.array(unnormalized).reshape(-1, 1), norm='l1', axis=0).ravel().tolist()
    for i in range(n_states):
        d[state_key[i]] = normalized[i]
    #return d

def updateProb(n_states, init, transition, emission):
    state_key = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    prob_dic = {}
    ...
    for i in range(n_states):
        ...
        for k in all_seq:
            prob_dic[newKey][k] = {}
            for s in tran_states:
                prob_dic[newKey][k][s] = init[s[0]]*emission[s[0]+k[0]]*transition[s]*emission[s[1]+k[1]]
    newProb = {}
    for k,i in prob_dic.items():
        numerator, denominator = 0.0, 0.0
        for s in tran_states:
            for k2,i2 in prob_dic[k].items():
                max_key = max(prob_dic[k][k2].items(), key=operator.itemgetter(1))[0]
                denominator += prob_dic[k][k2][max_key]
                numerator += getNumeratorForUP(k, prob_dic[k][k2].copy())
        newProb[k] = numerator/denominator
    normalize_prob_dic(n_states, newProb)          
    return newProb  
...
def Baum_Welch(n_states, n_emissions, init, transition, emission, sequence, n_iters=1000):
    for i in range(n_iters):
        ...
        newEmi = updateEmission(n_states, n_emissions, init, transition, emission)
        newProb = updateProb(n_states, init, transition, emission)
        transition = newTran
        emission = newEmi
        init = newProb
        #print(emission)
...
if __name__ == '__main__':
    ...
    #print(sequence)
    Baum_Welch(n_states, n_emissions, initial, transition, emission, sequence, int(sys.argv[1]))


    #newTran = updateTransition(n_states, initial, transition, emission, sequence)
    #print(transition)
    #print(newTran)
    #updateEmission(n_states, n_emissions, initial, transition, emission, sequence)